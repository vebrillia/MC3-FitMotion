...
//  MC3-FitMotion
//
//  Created by Vebrillia Santoso on 22/07/24.
//
...
struct CameraPositionView: View {
    ...
    var body: some View {
        NavigationView {
            ZStack {
                ...
                NavigationLink(destination: ExerciseView(), isActive: $navigateToExerciseView) {
                    EmptyView()
                }.navigationBarBackButtonHidden()
            }
            .onChange(of: cameraViewModel.recognizedPoints) { _ in
    ...
    func isUserInPosition() -> Bool {
        for point in cameraViewModel.recognizedPoints {
            if !boxFrame.contains(point) {
                return false
            }
        }
        return true
    ...
    func startCountdown() {
        isCountingDown = true
        audioManager.playSoundEffect(named: "CountDown") 
        timer?.invalidate()
